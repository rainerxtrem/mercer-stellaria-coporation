-- Eliminate module_slug ambiguity in API guard functions by using
-- distinct PL/pgSQL variable names.
...
BEGIN
  ...
  IF target_module_slug IS NULL THEN
    RETURN true;
  END IF;
    ...
    RETURN EXISTS (
      SELECT 1
        ...
        LEFT JOIN public.enterprise_modules module
         ...
         AND module.module_slug = target_module_slug
       WHERE membership.user_id = auth.uid()
         AND membership.status = 'active'
         AND (target_module_slug = 'clients' OR module.enabled = true)
    );
  END IF;
  ...
  IF active_firm_id IS NULL THEN
    RETURN false;
  END IF;
  RETURN app_private.user_has_module(auth.uid(), active_firm_id, target_module_slug);
END;
$$;
