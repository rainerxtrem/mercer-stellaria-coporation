...
import { Download, Send, ShieldAlert, Save } from "lucide-react";
...
import {
  getInsuranceClaim,
  getInsuranceClaimAttachmentUrl,
  listInsuranceClaims,
  sendInsuranceClaimMessage,
  updateInsuranceClaimStatus,
} from "@/lib/insurance-requests.functions";
import { Badge } from "@/components/ui/badge";

export const Route = createFileRoute("/_authenticated/sinistres")({
  head: () => ({ meta: [{ title: "Sinistres - Cabinet" }] }),
  component: SinistresStaffPage,
});

function SinistresStaffPage() {
  const qc = useQueryClient();
  const search = useRouterState({ select: (state) => state.location.search as Record<string, unknown> });
  const navigate = useNavigate();

  const listFn = useServerFn(listInsuranceClaims);
  const detailFn = useServerFn(getInsuranceClaim);
  const attachmentUrlFn = useServerFn(getInsuranceClaimAttachmentUrl);
  const statusFn = useServerFn(updateInsuranceClaimStatus);
  const sendMessageFn = useServerFn(sendInsuranceClaimMessage);

  const claimsQ = useQuery({ queryKey: ["staff", "claims"], queryFn: () => listFn() });
  const claimId = typeof search.request === "string" ? search.request : null;
  const detailQ = useQuery({
    queryKey: ["staff", "claim-detail", claimId],
  ...
  const selected = useMemo(() => claimsQ.data?.find((row: any) => row.id === claimId) ?? detail?.claim ?? null, [claimsQ.data, claimId, detail]);

  const statusMutation = useMutation({
    mutationFn: () => {
      if (!claimId) throw new Error("Aucun sinistre sélectionné.");
      return statusFn({
        data: {
          claim_id: claimId,
          status,
    ...
    onSuccess: async () => {
      ...
      await qc.invalidateQueries({ queryKey: ["staff", "claim-detail", claimId] });
    },
  ...
  const sendMessageMutation = useMutation({
    mutationFn: () => {
      if (!claimId || !reply.trim()) return Promise.resolve(null);
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-zinc-50">Sinistres</h1>
          <p className="text-sm text-zinc-400">Traitement des déclarations, statuts et échanges clients.</p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="border-zinc-800 bg-zinc-900/65">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-zinc-100"><ShieldAlert className="h-5 w-5 text-amber-300" /> Dossiers</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {(claimsQ.data ?? []).length === 0 && <p className="text-sm text-zinc-400">Aucun sinistre disponible.</p>}
            {(claimsQ.data ?? []).map((claim: any) => (
              <ClaimRow key={claim.id} claim={claim} claimId={claimId} navigate={navigate} />
            ))}
          </CardContent>
        </Card>
          ...
          <Card className="border-zinc-800 bg-zinc-900/65">
            ...
            <CardContent className="space-y-5 text-sm text-zinc-300">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                <Info label="Incident" value={selected.incident_date ?? "-"} />
                <Info label="Lieu" value={selected.incident_location ?? "-"} />
                <Info label="Type" value={selected.incident_type ?? "-"} />
                <Info label="Estimation" value={selected.estimated_amount ? `${Number(selected.estimated_amount).toFixed(2)} ${selected.currency}` : "-"} />
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="md:col-span-3">
                  ...
                  <Textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} rows={3} placeholder="Renseignez uniquement si le dossier est refusé" />
                </div>
              ...
              <div className="space-y-2 border-t border-zinc-800 pt-4">
                <Label htmlFor="claim-staff-reply">Réponse cabinet</Label>
                <Textarea id="claim-staff-reply" value={reply} onChange={(e) => setReply(e.target.value)} rows={4} placeholder="Répondre au client" />
...
function ClaimRow({ claim, claimId, navigate }: { claim: any; claimId: string | null; navigate: any }) {
  const clientLabel = [claim.clients?.first_name, claim.clients?.last_name].filter(Boolean).join(" ") || claim.clients?.company || "Client";
  return (
    <button
      onClick={() => navigate({ search: { request: claim.id } as any })}
      className={`w-full rounded-xl border p-4 text-left transition ${claimId === claim.id ? "border-amber-500/50 bg-amber-500/10" : "border-zinc-800 bg-zinc-950/70 hover:border-amber-500/30"}`}
    >
...
function AttachmentDownload({ attachmentId, label, fn }: { attachmentId: string; label: string; fn: any }) {
  const download = useMutation<any, Error, void>({
    mutationFn: async () => fn({ data: { attachment_id: attachmentId } }),
    onSuccess: (data: any) => window.open(data.url, "_blank", "noopener,noreferrer"),
  });
